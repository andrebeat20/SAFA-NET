...
export default function PaymentBottomSheet({ isOpen, onClose, customer, onPay }) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          {/* High performance dark backdrop overlay */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={(!isSubmitting && !isSuccess) ? onClose : undefined}
            className="fixed inset-0 bg-slate-950/70 z-[60]"
          />
          ...
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 bg-[var(--bg-secondary)] rounded-t-[32px] p-6 z-[70] pb-safe shadow-2xl border-t border-[var(--border-color)] transition-colors duration-500"
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                ...
                className="w-10 h-10 flex items-center justify-center bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
